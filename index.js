...
var logo = api[0].stream.logo;
var user = api[0].stream.display_name;
var desc = api[0].stream.status;
$("#desc").html(desc);
var lengthText = 20;
var text = $("#desc").text();
...
$("#desc").hover(function(){
  ...
  $("#logo").html("<img src='"+logo+"' height=40px width=40px>");
$("#user").html("<a href='https://www.twitch.tv/freecodecamp' target=blank>"+user+"</a>");
var ajax = $.ajax({
     type: "GET",
     url: 'https://wind-bow.gomix.me/twitch-api/streams/freecodecamp?callback=?',
     dataType: "json",
     success: function findStatus(data){
      var status = data.stream;
       if (status === undefined){
         $("#status").html("<em>Unavailable</em>");
       }
       else if(status === null){
      $("#status").html("<em>Not Live</em>");
}
  else {
  $("#status").html("<em>Online</em>");
}},
     error: function(errorMessage){
     alert("API not reached!");
   }
});
});